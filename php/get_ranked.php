...
//fetch the word that has as rank user s position+offset
	...
	$sql.=	") As sq WHERE sq.ID IS NOT NULL AND sq.DefinitionID IS NOT NULL AND sq.ID NOT IN (SELECT WordID FROM seengame".$mode." WHERE (userid=? OR userid=allusers) AND language = ?) AND sq.Rank = ? LIMIT 1;";
