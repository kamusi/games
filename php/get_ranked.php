...
//fetch the word that has as rank user s position+offset
	$sql =  "SELECT ID As ID, DefinitionID As DefinitionID, Rank As Rank FROM (";
		...
		$sql.=	") As sq WHERE sq.ID IS NOT NULL AND sq.DefinitionID IS NOT NULL sq.ID NOT IN (SELECT WordID FROM wordsAlreadySeenMode1 WHERE UserID=?) AND sq.Rank = ? LIMIT 1;";
